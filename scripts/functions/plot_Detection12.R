plot_Detection12 <- function(model, spname = NULL, min.yr = NULL, CI=95){
  ...
  #Plot detection for last time period
  gp2 <- ggplot(data = subset(pDet, year==12)) +
    geom_violin(aes(x=as.factor(ListLength), y=pDet), alpha=0.2) +
