...
This is the script to create figure 2 of the paper, illustrating occupancy change for each species from 1900-1909 to 2000-2019
...
(Inc_plot<-ggplot(data=Increasing, aes(x=TP, y=Occupancy, group = Species, color=Species, fill=Species)) +
  ...
  labs(y="Occupancy", x = "")+
  scale_color_manual(values = c("#543005", "#66c2a4", "#67A9CF", "#D94801")) +
...
(Mix_plot<-ggplot(data=Fluctuating, aes(x=TP, y=Occupancy, group = Species, color=Species, fill=Species)) +
    ...
    scale_color_manual(values = c("#99d8c9", "#3690c0", "#2A91A6", "#015C63", "#9e9ac8", "#824B08")) +
    geom_dl(aes(label = ID), method = list("last.bumpup",
          cex = 0.9)) +
    scale_y_continuous(limits=c(0,1.1), breaks=seq(0,1,0.1)) +
    theme_classic(base_size = 12)+
...
```{r}
png("Fig2.png", width = 1200, height = 750)
